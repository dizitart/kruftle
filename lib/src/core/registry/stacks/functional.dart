...
const elmStack = StackDefinition(
  id: StackId.elm,
  displayName: 'Elm',
  markers: {'elm.json'},
  // No official clean command; elm-stuff/ is fully regenerated by the next
  // `elm make`/`elm test` invocation, so this is allow-listed raw deletion
  // only -- no `tool`/`cleanCommand` at all.
  artifacts: [ArtifactPath('elm-stuff', risk: CleanRisk.cache)],
  priority: 20,
);
